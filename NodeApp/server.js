    ...
    if(err) console.log(err);
      ...
      function (err, user_liked, fields){
        if(err) console.log(err);
          ...
          musicdb.query('select * from songs where popularity > 80 group by artist_id order by popularity desc limit 8', function (err, songs, fields) {
              if(err)
                console.log(err);
              else{
                  musicdb.query('select albums.popularity as popularity, image_url as image_url, album_name, artist_name, albums.redirect_url as redirect_url from albums,artists where albums.artist_id = artists.artist_id and albums.popularity > 80 order by albums.popularity desc limit 5', function (err, albums, fields){
                    if(err)
                      console.log(err);
                    else{
                      musicdb.query('select * from songs, tag_for_song where songs.track_id = tag_for_song.track_id and tag_for_song.tag_id in (select tag_id from tag_for_song, prefered_songs where tag_for_song.track_id = prefered_songs.track_id and user_id = ? ) group by artist_id order by popularity desc limit 5', login_userid, function (err, recommendedtracks, fields){
                        if(err)
                          console.log(err);
                        else {
                          // console.log(recommendedalbums);
                            musicdb.query('select * from albums, tag_for_album where albums.album_id = tag_for_album.album_id and tag_for_album.tag_id in (select tag_id from tag_for_song, prefered_songs where tag_for_song.track_id = prefered_songs.track_id and user_id = 11 ) group by albums.album_name order by popularity desc limit 12', function (err, recommendedalbums, fields){
                              if(err)
                                console.log(err);
                              else {
...
app.get('/addtolist/*', function (req, res) {
  ...
  var redirect_url = req.headers.referer;
  musicdb.query('select * from listed_songs where track_id = ? and list_id=?', [track_id, list_id], function (err, rows, fields){
    if(err) console.log(err);
    else if(!rows.length){
